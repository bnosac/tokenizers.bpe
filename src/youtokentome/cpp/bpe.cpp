...
string fast_read_file_utf8(const string &file_name) {
  ...
  if (fin == nullptr) {
    Rcpp::Rcerr << "Error. Failed to open file: " + file_name << std::endl;
    Rcpp::stop("Exiting");
  }
  while (true) {
...
string token2word(const vector<uint32_t> &source,
                  const phmap::flat_hash_map<uint32_t, uint32_t> &id2char) {
  vector<uint32_t> res;
  for (int i : source) {
...
phmap::flat_hash_map<uint32_t, uint32_t> compute_alphabet_helper(
    const phmap::flat_hash_map<uint32_t, size_t> &char_cnt, size_t data_len,
    phmap::flat_hash_set<uint32_t> &removed_chars, const BpeConfig &bpe_config) {
  ...
  Rcpp::Rcerr << "number of unique characters in the training data: "
              << frequencies.size() << std::endl;
  Rcpp::Rcerr << "number of deleted characters: " << cur << std::endl;
  Rcpp::Rcerr << "number of unique characters left: " << frequencies.size() - cur
              << std::endl;
...
void remove_rare_chars(vector<uint32_t> &data,
                       const phmap::flat_hash_set<uint32_t> &removed_chars) {
...
phmap::flat_hash_map<VectorSegment, WordCount> compute_word_count_helper(
    const vector<uint32_t> &row_data,
    const phmap::flat_hash_map<uint32_t, uint32_t> &char2id) {
  phmap::flat_hash_map<VectorSegment, WordCount> hash2wordcnt;
  vector<uint32_t> word;
...
void build_linked_list(const vector<WordCount> &word_cnt,
                       ...
                       phmap::flat_hash_map<uint64_t, vector<Position>> &pair2pos,
                       phmap::flat_hash_map<uint64_t, size_t> &pair2cnt) {
...
void init_recipe(const phmap::flat_hash_map<uint32_t, uint32_t> &char2id,
                 phmap::flat_hash_map<uint32_t, vector<uint32_t>> &recipe,
                 phmap::flat_hash_map<uint32_t, string> &recipe_s) {
...
void worker_doing_merge(
    ...
    vector<phmap::flat_hash_map<uint64_t, size_t>> &pair2cnt_g,
    phmap::flat_hash_map<uint64_t, vector<Position>> &pair2pos,
    ...
    phmap::flat_hash_map<uint32_t, uint32_t> &char2id,
    vector<vector<phmap::flat_hash_map<uint32_t, size_t>>> &left_tokens_submit,
    vector<vector<phmap::flat_hash_map<uint32_t, size_t>>> &right_tokens_submit,
  ...
  phmap::flat_hash_set<uint32_t> left_tokens;
  phmap::flat_hash_set<uint32_t> right_tokens;
...
void rename_tokens(phmap::flat_hash_map<uint32_t, uint32_t> &char2id,
                   vector<BPE_Rule> &rules, const SpecialTokens &special_tokens,
                   uint32_t n_tokens) {
  phmap::flat_hash_map<uint32_t, uint32_t> renaming;
...
BPEState learn_bpe_from_string(string &text_utf8, int n_tokens,
  ...
  vector<phmap::flat_hash_map<uint32_t, size_t>> shared_char_cnt(n_threads);
  ...
  phmap::flat_hash_set<uint32_t> removed_chars;
  phmap::flat_hash_map<uint32_t, uint32_t> char2id;

  vector<phmap::flat_hash_map<VectorSegment, WordCount>> hash2wordcnt(n_threads);

  phmap::flat_hash_map<uint32_t, vector<uint32_t>> recipe;
  phmap::flat_hash_map<uint32_t, string> recipe_s;
  vector<phmap::flat_hash_map<uint64_t, size_t>> pair2cnt_g(n_threads);
  phmap::flat_hash_map<uint64_t, vector<Position>> pair2pos;
  ...
  vector<vector<phmap::flat_hash_map<uint32_t, size_t>>> left_tokens_submit(
      2, vector<phmap::flat_hash_map<uint32_t, size_t>>(n_threads));
  vector<vector<phmap::flat_hash_map<uint32_t, size_t>>> right_tokens_submit(
      2, vector<phmap::flat_hash_map<uint32_t, size_t>>(n_threads));
  ...
  for (size_t i = 0; i < n_threads; i++) {
    threads.emplace_back(
      [&](size_t thread_id) {
        ...
        phmap::flat_hash_map<uint32_t, size_t> char_cnt;
        utf_code_points =
  ...
  phmap::flat_hash_map<uint64_t, uint64_t> real_pair_cnt;
  ...
  phmap::flat_hash_map<uint32_t, size_t> all_res;
  vector<char> local_check_list(n_threads);
  phmap::flat_hash_map<uint32_t, size_t> global_ht_update_left;
  phmap::flat_hash_map<uint32_t, size_t> global_ht_update_right;
        ...
        while (true) {
          if (merge_order.empty()) {
            if (finished_cur == used_ids) {
              Rcpp::Rcerr << "WARNING merged only: " << used_ids
  ...
  Rcpp::Rcerr << "model saved to: " << output_file << std::endl;
  return bpe_state;
}

void build_linked_list(
    const vector<WordCount> &word_cnt, vector<vector<NodeEncoder>> &list,
    phmap::flat_hash_map<uint64_t, PositionsCnt> &pair2poscnt) {
  list.resize(word_cnt.size());
  for (size_t i = 0; i < word_cnt.size(); i++) {
    for (uint32_t ch : word_cnt[i].word) {
      if (!list[i].empty() && list[i].back().val == ch) {
...
phmap::flat_hash_map<uint32_t, uint32_t> compute_alphabet(
    const vector<uint32_t> &data, phmap::flat_hash_set<uint32_t> &removed_chars,
...
void check_config(BpeConfig &bpe_config, int vocab_size) {
  if (bpe_config.character_coverage <= 0 || bpe_config.character_coverage > 1) {
    Rcpp::Rcerr << "Invalid value. Character_coverage value must be in the range "
    "(0, 1]."
    << std::endl;
    Rcpp::stop("Exiting");
  }
  if (bpe_config.special_tokens.unk_id < 0 ||
    ...
    Rcpp::Rcerr << "Invalid value. unk_id: must be in the range [0, vocab_size - "
  ...
  if (bpe_config.special_tokens.pad_id < -1 ||
    ...
    Rcpp::Rcerr << "Invalid value. pad_id must be in the range [-1, vocab_size - "
    "1]. Current value of vocab_size = " +
      std::to_string(vocab_size) + "."
    << std::endl;
      Rcpp::stop("Exiting");
  ...
  if (bpe_config.special_tokens.bos_id < -1 ||
    ...
    Rcpp::Rcerr << "Invalid value. bos_id must be in the range [-1, vocab_size - "
    "1]. Current value of vocab_size = " +
      std::to_string(vocab_size) + "."
    << std::endl;
      Rcpp::stop("Exiting");
  ...
  if (bpe_config.special_tokens.eos_id < -1 ||
    ...
    Rcpp::Rcerr << "Invalid value. eos_id must be in the range [-1, vocab_size - "
    "1]. Current value of vocab_size = " +
      std::to_string(vocab_size) + "."
    << std::endl;
      Rcpp::stop("Exiting");
  ...
  cnt_add++;
  if (ids.size() != cnt_add) {
    Rcpp::Rcerr << "Error: All ids of special tokens must be different."
                << std::endl;
    Rcpp::stop("Exiting");
  }
...
void print_config(const string &input_path, const string &model_path,
  ...
  Rcpp::Rcerr << "Training parameters" << std::endl;
  Rcpp::Rcerr << "  input: " << input_path << std::endl;
  Rcpp::Rcerr << "  model: " << model_path << std::endl;
  Rcpp::Rcerr << "  vocab_size: " << vocab_size << std::endl;
  Rcpp::Rcerr << "  n_threads: " << bpe_config.n_threads << std::endl;
  Rcpp::Rcerr << "  character_coverage: " << bpe_config.character_coverage
              << std::endl;
  Rcpp::Rcerr << "  pad: " << bpe_config.special_tokens.pad_id << std::endl;
  Rcpp::Rcerr << "  unk: " << bpe_config.special_tokens.unk_id << std::endl;
  Rcpp::Rcerr << "  bos: " << bpe_config.special_tokens.bos_id << std::endl;
  Rcpp::Rcerr << "  eos: " << bpe_config.special_tokens.eos_id << std::endl;
  Rcpp::Rcerr << std::endl;
}
...
void train_bpe(const string &input_path, const string &model_path,
  ...
  Rcpp::Rcerr << "reading file..." << std::endl;
  auto data = fast_read_file_utf8(input_path);
  Rcpp::Rcerr << "learning bpe..." << std::endl;
...
DecodeResult BaseEncoder::encode_sentence(const std::string &sentence_utf8,
  ...
  phmap::flat_hash_map<uint32_t, string> unrecognized_tokens;
...
std::vector<DecodeResult> BaseEncoder::encode_parallel(
  ...
  if (encoding_config.bos && bpe_state.special_tokens.bos_id == -1) {
    Rcpp::Rcerr << "Can't add <BOS> token. Model was trained without it."
                << std::endl;
    Rcpp::stop("Exiting");
  }
  if (encoding_config.eos && bpe_state.special_tokens.eos_id == -1) {
    Rcpp::Rcerr << "Can't add <EOS> token. Model was trained without it."
                << std::endl;
    Rcpp::stop("Exiting");
  }
...
string BaseEncoder::id_to_subword(int id, bool replace_space) const {
  if (id < 0 || vocab_size() <= id) {
    Rcpp::Rcerr << "Error: Invalid value for id. id must be in the range [0, "
...
void BaseEncoder::vocab_cli(bool verbose) const {
  ...
  phmap::flat_hash_map<uint32_t, std::pair<uint32_t, uint32_t>> reversed_rules;
  if (verbose) {
    for (auto rule : bpe_state.rules) {
  ...
  for (size_t i = 0; i < n_tokens; i++) {
    ...
    if (verbose) {
      if (reversed_rules.count(i)) {
        ...
        Rcpp::Rcout << "=" << token_x << "+" << token_y;
        for (int t = 0; t < std::max(2, 50 - used_symbols); t++) {
          Rcpp::Rcout << " ";
        }
        Rcpp::Rcout << comb.first << "+" << comb.second;
      }
    }
    Rcpp::Rcout << std::endl;
  }
...
void BaseEncoder::encode_cli(const string &output_type_str, bool stream,
  ...
  if (stream) {
    ...
    size_t processed;
    Rcpp::Rcerr << "n_threads: " << n_threads << std::endl;
    int chars_remove = 0;
    do {
      processed = 0;
      ...
      Rcpp::Rcerr << message << total_progress;
